...
import RoomStatusModal from './Partials/RoomStatusModal.vue';
...
// 🎯 ROOM STATUS MODAL
const isRoomStatusModalOpen = ref(false);
const selectedRoomForStatusUpdate = ref(null);

const openRoomStatusModal = (room) => {
    selectedRoomForStatusUpdate.value = room;
    isRoomStatusModalOpen.value = true;
};
        ...
        <div class="space-y-6 pb-12 animate-in fade-in duration-500">
            ...
            <template v-if="activeTab === 'matrix'">
                ...
                <div class="flex flex-wrap gap-6 text-[10px] font-black uppercase tracking-widest text-muted-text bg-white dark:bg-dark-card p-4 rounded-2xl border border-slate-100 dark:border-dark-border shadow-sm inline-flex mt-4">
                    ...
                    <div class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-orange-500"></span> Đang dọn dẹp</div>
                    <div class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-purple-500"></span> Đang bảo trì</div>
                </div>
                ...
                <div v-for="(rooms, floor) in filteredGroupedRooms" :key="floor" class="space-y-4 mt-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
                        <div v-for="room in getPaginatedMatrixRoomsByFloor(floor)" :key="room.id"
                             @click="getCurrentBooking(room) ? openDetailModal(getCurrentBooking(room)) : openRoomStatusModal(room)"
                             class="relative p-6 rounded-[2.5rem] border-2 transition-all group overflow-hidden flex flex-col cursor-pointer"
                             :class="{
                                 'bg-white dark:bg-dark-card border-slate-50 dark:border-dark-border hover:border-primary-500/30 shadow-sm': !getCurrentBooking(room) && room.status === 'available',
                                 'bg-slate-200 border-slate-400 dark:bg-slate-800 dark:border-slate-600 hover:border-slate-500 shadow-sm': !getCurrentBooking(room) && room.status === 'occupied',
                                 'bg-orange-50 border-orange-300 dark:bg-orange-900/20 dark:border-orange-500/30 hover:border-orange-400 shadow-sm': !getCurrentBooking(room) && room.status === 'cleaning',
                                 'bg-purple-50 border-purple-300 dark:bg-purple-900/20 dark:border-purple-500/30 hover:border-purple-400 shadow-sm': !getCurrentBooking(room) && room.status === 'maintenance',
                            ...
                            <div class="flex justify-between items-start mb-4">
                                ...
                                <div class="w-2.5 h-2.5 rounded-full shrink-0"
                                     :class="{
                                         ...
                                         'bg-slate-500 shadow-[0_0_8px_rgba(100,116,139,0.5)]': !getCurrentBooking(room) && room.status === 'occupied',
                                         'bg-orange-500': room.status === 'cleaning',
                                         'bg-purple-500': room.status === 'maintenance',
                                         'bg-slate-300': !getCurrentBooking(room) && room.status === 'available'
                                     }">
                                </div>
                            </div>

                            <!-- Room Status Badge (for empty rooms) -->
                            <div v-if="!getCurrentBooking(room)" class="mb-3">
                                <span v-if="room.status === 'cleaning'" class="inline-block px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest bg-orange-500/20 text-orange-600 dark:bg-orange-500/30 dark:text-orange-400 border border-orange-200 dark:border-orange-500/50">
                                    🧹 Đang dọn dẹp
                                </span>
                                <span v-else-if="room.status === 'maintenance'" class="inline-block px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest bg-purple-500/20 text-purple-600 dark:bg-purple-500/30 dark:text-purple-400 border border-purple-200 dark:border-purple-500/50">
                                    🔧 Đang bảo trì
                                </span>
                                <span v-else-if="room.status === 'occupied'" class="inline-block px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest bg-slate-700/15 text-slate-700 dark:bg-slate-500/20 dark:text-slate-300 border border-slate-300 dark:border-slate-500/40">
                                    🛏 Đang sử dụng
                                </span>
                                <span v-else class="inline-block px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest bg-emerald-500/20 text-emerald-600 dark:bg-emerald-500/30 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/50">
                                    ✓ Trống
                                </span>
                            </div>
                            ...
                            <div v-else class="mt-auto flex flex-col gap-2 pt-4 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">
                                ...
                                <button v-if="room.status !== 'occupied' || !getCurrentBooking(room)" @click.stop="openRoomStatusModal(room)"
                                        class="text-[11px] font-black text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 py-2.5 rounded-xl text-center hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors uppercase tracking-widest">
                                    ⚙️ Đổi Trạng Thái
                                </button>
                            </div>
                        </div>
            ...
            <RoomStatusModal
                v-if="selectedRoomForStatusUpdate"
                :show="isRoomStatusModalOpen"
                :room="selectedRoomForStatusUpdate"
                @close="isRoomStatusModalOpen = false"
            />
